...
V3.19.5 restored Gemini/Veo as the primary video engine (gated on a clean ASCII `GEMINI_API_KEY`); Replicate (`minimax/hailuo-2.3-fast`), fal.ai and HF spaces form the automatic fallback chain. The Stars-paid `Оживить последнее фото` flow unlocks when any engine is configured, with automatic Stars refund attempts when generation fails.
