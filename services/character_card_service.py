...
DEFAULT_CARDS = {
    "anna_01": {
        ...
        "gender": "female",
        "age": 26,
        "short_bio": "Горячая смесь нежности и страсти. Знает, как одним взглядом заставить тебя забыть обо всём. Любит долгие поцелуи, вечерний город и игры на грани.",
        "status": "active",
        "button_emoji": "👩🏻",
        "is_visible": True,
    },
    "alena_01": {
        ...
        "gender": "female",
        "age": 25,
        "short_bio": "Дерзкая блондинка с характером. Носит мини, ездит быстро, говорит прямо. Если не боишься потерять голову — тебе к ней.",
        "status": "active",
        "button_emoji": "👱‍♀️",
        "is_visible": True,
    },
    ...
    "erika_01": {
        ...
        "gender": "female",
        "age": 38,
        "short_bio": "Женщина, которая знает, чего хочет. Мать твоего друга — но это не мешает ей смотреть на тебя так, что забываешь своё имя. Опыт, от которого кружится голова.",
        "status": "active",
        "button_emoji": "🌹",
        "is_visible": True,
    },
    "sonya_01": {
        ...
        "gender": "female",
        "age": 22,
        "short_bio": "Твоя соседка детства. Помнит все твои секреты. Выросла — и теперь дразнит так, что хочется забыть, что вы «просто друзья». Мини-юбки и невинный взгляд — её оружие.",
        "status": "active",
        "button_emoji": "",
        "is_visible": True,
    },
    "vika_01": {
        ...
        "gender": "female",
        "age": 35,
        "short_bio": "Начальница, от которой мурашки. Строгая на людях, но остаётся с тобой допоздна. И снимает пиджак. Подчиняться ещё никогда не было так приятно.",
        "status": "active",
        "button_emoji": "👠",
        "is_visible": True,
    },
    "alisa_01": {
        ...
        "gender": "female",
        "age": 27,
        "short_bio": "Учительница, которая задерживается после уроков. Тихая, умная, с очками. Но стоит остаться вдвоём — и она показывает, что скрывалось за каждой оценкой.",
        "status": "active",
        "button_emoji": "📚",
        "is_visible": True,
    },
    "mila_01": {
        ...
        "gender": "female",
        "age": 30,
        "short_bio": "Соседка, которая стучит в дверь с пирогом. Заботливая, уютная. Но когда заходишь — понимает, что пирог был только предлог. А домашний халат — случайно.",
        "status": "active",
        "button_emoji": "☕",
        "is_visible": True,
    },
...
SCENARIO_HOOKS = {
    ...
    "erika_01": (
        "Ты зашёл к другу, а его дома нет — встречает мама. Эрика в шёлковой блузке наливает тебе вина: "
        "«Погоди его здесь… или расскажи мне лучше, как ты живёшь. Мне правда интересно.»"
    ),
    "sonya_01": (
        "«Смотри, я переехала! И да — я всё ещё помню, кто съел моё мороженое в девятом классе. "
        "Приезжай сегодня, я покажу тебе город. Ну или хотя бы район. Ну или свою кухню…»"
    ),
    "vika_01": (
        "Офис опустел, только у Вики горит свет. Она снимает пиджак и кладёт перед тобой папку: "
        "«Останься. Нам нужно… многое обсудить. И не про работу.»"
    ),
    "alisa_01": (
        "Алиса задержалась после уроков — кардиган, очки, гора тетрадей. Она вдруг краснеет: "
        "«Я как раз проверяла твою работу… останься на минутку? Хочу сказать кое-что не по учебе.»"
    ),
    "mila_01": (
        "Стук в дверь — Мила с дымящимся пирогом: «Опять не ел, я же вижу. "
        "Заходи, чайник уже кипит… и расскажи мне, почему ты сегодня такой грустный.»"
    ),
}
...
def ensure_default_cards() -> None:
    with SessionLocal() as session:
        ...
        _old_bios = {
            ...
            "anna_01_v2": "Тёплая, очень общительная, соблазнительная и страстная. Любит вечерний город, музыку, стильные образы, дерзкие намёки и сексуальное напряжение.",
            "alena_01_v2": "Яркая, дерзкая и очень сексуальная. Любит street fashion, автомобили, спонтанные поездки и провокационные разговоры. Не боится быть откровенной и дразнить.",
        }
        for character_id, defaults in DEFAULT_CARDS.items():
            ...
            if character_id == "alena_01" and (row.display_name or "").strip() in {"Алёна", "Алена"}:
                ...
                changed = True
            # Update bios that still hold old default text (any version).
            _all_old = set(_old_bios.values())
            if row.short_bio and row.short_bio in _all_old:
