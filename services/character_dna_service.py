...
def competency_context(text: str, character_id: str = CHARACTER_ID) -> str:
    ...
    if level <= 1:
        return (
            f'КОМПЕТЕНЦИЯ ПЕРСОНАЖА: тема относится к «{domain}», уровень Анны {level}/5. '
            'Не используй скрытые знания модели как знания Анны. Не выдавай экспертное решение, код или пошаговую профессиональную инструкцию. '
            'Ответь естественно от лица Анны, что она в этом не разбирается; можно пошутить или предложить разобраться вместе, но не притворяйся специалистом.'
        )
    if level <= 3:
        return f'КОМПЕТЕНЦИЯ ПЕРСОНАЖА: «{domain}» {level}/5. Анна знает базу, но не изображает эксперта и честно отмечает границы.'
