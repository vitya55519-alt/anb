...
async def _rewrite_if_needed(messages: list[dict], user_text: str, answer: str) -> str:
    ...
    rewrite_messages = messages + [
        ...
        {"role": "system", "content": (
            "Перепиши последний ответ Анны. Сохрани смысл, но сделай его естественным личным сообщением. "
    ...
    r = await client.chat.completions.create(model=AI_MODEL, messages=rewrite_messages, temperature=0.65, max_tokens=280)
...
async def reply(user_id: int, user_name: str, user_text: str) -> str:
    ...
    if test_stage:
        ...
        stage_prompts = {
            'stranger': 'Уровень 1 — знакомство. Лёгко и без фамильярности.',
            'acquaintance': 'Уровень 2 — уже знакомые. Больше узнавания и лёгких подколов.',
            'close': 'Уровень 3 — близкое общение. Теплее, больше callbacks и инициативы.',
            'intimate': 'Уровень 4 — доверие и активный флирт, но не в каждом сообщении.',
    ...
    r = await client.chat.completions.create(model=AI_MODEL, messages=messages, temperature=0.9, max_tokens=320)
...
async def proactive_reply(user_id: int, user_name: str, hours_inactive: int) -> str:
    ...
    r = await client.chat.completions.create(model=AI_MODEL, messages=messages, temperature=0.95, max_tokens=100)
    answer = _clean(r.choices[0].message.content)
    save_message(db_user_id, CHARACTER_ID, "assistant", answer)
    return answer
