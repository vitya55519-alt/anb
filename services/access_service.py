...
def is_premium(telegram_id: int) -> bool:
    ...
    with SessionLocal() as s:
        ...
        if not user:
            return False
        return bool(s.scalar(select(func.count()).select_from(Subscription).where(
...
def can_send_message(telegram_id: int) -> bool:
    ...
    with SessionLocal() as s:
        ...
        if not user:
            return True
        count = s.scalar(select(func.count()).select_from(Message).where(
            Message.user_id == user.id, Message.role == "user", Message.created_at >= start
        )) or 0
